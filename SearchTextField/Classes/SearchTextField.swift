...
open class SearchTextField: UITextField {
        ...
        for i in 0 ..< filterDataSource.count {
            ...
            if !inlineMode {
                ...
                if item.title.lowercased().hasPrefix(textToFilter) {
                    let itemSuffix = item.title.substring(from: textToFilter.index(textToFilter.startIndex, offsetBy: textToFilter.count))
    ...
    fileprivate func highlightAttributesForSubtitle() -> [NSAttributedStringKey: AnyObject] {
        ...
        for attr in highlightAttributes {
            if attr.0.rawValue == NSAttributedStringKey.font.rawValue {
